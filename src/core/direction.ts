...
/**
 * Quantizes an angle measured clockwise from screen-up into one of 16 sectors.
 * Each sector is centered on a multiple of 22.5 degrees.
 */
...
/**
 * Quantizes a screen-space vector. Zero-length vectors and vectors inside the
 * optional dead zone keep the previous pose by returning null.
 */
export function quantizeScreenDirection(
  ...
  if (Math.hypot(deltaX, deltaY) <= deadZonePx) {
    return null;
  }
